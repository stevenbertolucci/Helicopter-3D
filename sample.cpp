...
// Course: CS 450 - Intro to Graphics
// Project: Project 2 - Transformations (Animate a Helicopter!)
...
float	TopBladeAngle = 0.0;
float	RearBladeAngle = 0.0;
...
Display( )
{
	...
	if (ModeOn == 1) {
		...
		// Outside view
		gluLookAt(0.f, 3.f, 10.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f);		
	}
	...
	// Draw the rotating top blade at (0., 2.9, -2.)
	glPushMatrix();
	...
	// Draw the rotating rear blade at (.5, 2.5, 9.)
	glPushMatrix();
...
InitLists( )
{
	...
	// create the rear blade:
	RearBladeList = glGenLists(1);
	glNewList(RearBladeList, GL_COMPILE);

	glBegin(GL_TRIANGLES);
	glVertex2f(REAR_BLADE_RADIUS, BLADE_WIDTH / 2.);
	glVertex2f(0., 0.);
	glVertex2f(REAR_BLADE_RADIUS, -BLADE_WIDTH / 2.);
